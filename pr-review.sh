...
Common options:
  ...
  --host <HOST>            GitHub host, e.g. github.com or sgithub.world.socgen
...
post options:
  ...
  --no-verify-paths        skip checking comment paths against changed files
...
config_value() {
  ...
  [[ -z "$CONFIG_FILE" ]] && { printf '%s' "$fallback"; return; }
...
load_config() {
  ...
  if [[ -n "$CONFIG_FILE" ]] && jq -e '.fetch.excludePaths | type == "array"' "$CONFIG_FILE" >/dev/null 2>&1; then
    local pattern
    while IFS= read -r pattern; do
      [[ -n "$pattern" ]] && EXCLUDE_PATTERNS+=("$pattern")
    done < <(jq -r '.fetch.excludePaths[]' "$CONFIG_FILE")
  else
...
cmd_fetch() {
  ...
  if [[ "$INCLUDE_DIFF" == "true" ]]; then
    if gh_cmd pr diff "$PR_NUMBER" --repo "$REPO" >"$raw_diff" 2>"$TMP_DIR/diff.err"; then
      awk -v exclude_file="$excluded_list" '
        BEGIN { while ((getline line < exclude_file) > 0) excluded[line] = 1 }
        /^diff --git / {
          marker = index($0, " b/")
  ...
  if [[ -n "$CONFIG_FILE" ]]; then
    jq '.review // {}' "$CONFIG_FILE" >"$review_config"
  else
    printf '{}' >"$review_config"
  fi
